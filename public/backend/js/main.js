...
function destroyProduct(id) {
    ...
    if (result) { // neu nhấn == ok , sẽ send request ajax, dc r.
        $.ajax({
            ...
            type: 'DELETE',
            // data: {}, // dữ liệu truyền sang nếu có
            data: {_token: $("meta[name=csrf-token]").attr("content")}, // dữ liệu truyền sang nếu có
            dataType: "json", // kiểu dữ liệu trả về
            success: function (response) { // success : kết quả trả về sau khi gửi request ajax
                // dữ liệu trả về là một object nên để gọi đến status chúng ta sẽ gọi như bên dưới
                if (response.status != 'undefined' && response.status == true) {
...
function destroyPost (id) {
    ...
    if (result) { // neu nhấn == ok , sẽ send request ajax, dc r.
        $.ajax({
            url: base_url + '/admin/post/'+id, // base_url được khai báo ở đầu page == http://webshop.local
            type: 'DELETE',
            // data: {}, // dữ liệu truyền sang nếu có
            data: {_token: $("meta[name=csrf-token]").attr("content")}, // dữ liệu truyền sang nếu có
            dataType: "json", // kiểu dữ liệu trả về
